...
const CreateFolderModal = ({ show, onClose, contract, folder }) => {
  ...
  const _handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const _folder = await models.Folder.create({ title, parent_id: folder?.id || contract.folder_id });
      onClose(_folder);
    } catch (e) {
      setLoading(false);
          ...
          >
            <div className="inline-block w-full max-w-xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
              <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-gray-900 mb-2">
                Ajouter un sous-dossier dans "{folder?.title || contract.title}"
              </Dialog.Title>
              <form
